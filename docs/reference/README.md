...
sometimes by a lot. Read everything as a *ratio* (this curve spans about
...
*visual language* — the light-forms, the layering, the pacing, the way type sits
...
When in doubt: take the *structure*, leave the *identity*.

**3. Take the treatment, replace the subject.** This is the most important line
in this file. Every form in the film is a *VPN* metaphor — a globe because they
...
looks expensive and says nothing true about us. What transfers is the *craft*:
one enormous luminous object per section, lit from behind, held on a quiet
ground, with the type kept small and calm beside it. What the object *is* has to
...
and it is why the 3D forms are lit from *behind* into a silhouette with a
...
| `04-data-protection-comet.jpg` | 9.4s | Large sphere, a comet streaking across it, headline below, two small cards | One hero object dead centre with a single moving highlight, type *below* it, small cards under that. Fits `ServicesSection` |
...
| `07-many-locations-globe.jpg` | 15.4s | Dot-matrix globe, two stats flanking it, headline below | Object built from *many small elements*, with figures set either side of it. Fits `PlaceSection` |
...
- **Something being assembled** — planes, panels or blocks converging into one
  solid, or a wireframe resolving into a surface. Reads as *building software*
...
- **A lattice of nodes and links** — the dot-globe's *technique* (many small
...
not a fully-lit grey model. Variety should come from the *forms*, not from
