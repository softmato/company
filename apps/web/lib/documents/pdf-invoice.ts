...
export async function drawInvoice(
  ...
  if (document.issuedAt)
    meta.push(['Invoice Date', dateLine(document.issuedAt)]);
  ...
  partyBlock(sheet, 'Bill to', document.customer, {
    showAbsentPan: true,
    width: 280,
  });
  ...
  const badge = sheet.stamp(
    RIGHT - 180,
    sheet.y + 4,
    STATUS_LABEL[document.status],
    STATUS_INK[document.status],
  );
  ...
  for (const line of document.lines) {
    const described = sheet.wrap(line.description, DESCRIPTION_WIDTH, {
      size: 9,
    });
    const period =
    ...
    sheet.right(COL.qty, trimQuantity(line.quantity), {
      face: 'mono',
      size: 8.5,
    });
    sheet.right(COL.rate, formatPaisa(line.unitPriceMinor), {
      face: 'mono',
      size: 8.5,
    });
    sheet.right(RIGHT, formatPaisa(line.amountMinor), {
      face: 'mono',
      size: 8.5,
    });
  ...
  if (document.lines.length === 0) {
    sheet.text(COL.description, 'This invoice has no lines recorded.', {
      color: INK.faint,
      size: 9,
    });
    sheet.down(8);
  ...
  if (document.discountMinor > 0n)
    rows.push(['Discount', `-${formatPaisa(document.discountMinor)}`, 'plain']);
  if (document.taxMinor > 0n)
    rows.push(['Tax', formatPaisa(document.taxMinor), 'plain']);
  ...
  for (const [label, value, weight] of rows) {
    ...
    sheet.text(totalsLeft, label, {
      color: bold ? INK.text : INK.soft,
      face: bold ? 'bold' : 'sans',
      size,
    });
  ...
  if (plan) {
    ...
    sheet.eyebrow(
      MARGIN.left,
      `${plan.plan_name}${plan.billing_period ? ` · ${plan.billing_period}` : ''}`,
    );

    if (plan.tagline)
      sheet.paragraph(MARGIN.left, plan.tagline, RIGHT - MARGIN.left, {
        color: INK.soft,
        size: 9,
      });
    ...
    for (const feature of plan.features ?? []) {
      ...
      sheet.paragraph(MARGIN.left + 12, feature, RIGHT - MARGIN.left - 12, {
        size: 9,
      });
    }
  ...
  sheet.ensure(46);
  sheet.rule().down(16);
  sheet.paragraph(MARGIN.left, PAY_NOTE, RIGHT - MARGIN.left, {
    color: INK.soft,
    size: 8,
  });
  sheet.paragraph(
