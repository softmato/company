    ...
    <div className="float-card w-[23rem] max-w-full overflow-hidden">
      <div className="flex items-center gap-3 border-b border-border bg-surface px-3.5 py-2.5">
        ...
        <span className="flex flex-1 items-center justify-center gap-1.5 rounded-md bg-card py-1 text-[11.5px] text-muted-foreground">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" className="size-3" aria-hidden="true">
      ...
      <div className="p-4">
        <div className="flex items-center justify-between">
          <p className="text-[14px] font-medium text-foreground">{PORTAL.title}</p>
            ...
            <li key={project.name} className="flex items-center gap-3">
              ...
              <span className="min-w-0 flex-1">
                <span className="flex items-baseline justify-between gap-2">
                  <span className="truncate text-[12.5px] font-medium text-foreground">{project.name}</span>
                  <span
                    className={cn(
                      'flex-none text-[11px] font-medium',
                      project.progress === 100 ? 'text-primary' : 'text-muted-foreground',
                    )}
                  >
                    {project.stage}
                  </span>
        ...
        <div className="mt-4 flex items-center gap-2.5 rounded-xl bg-primary/8 p-2.5">
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-[12px] font-medium text-foreground">{PORTAL.update.title}</span>
            <span className="block truncate text-[11.5px] text-muted-foreground">{PORTAL.update.body}</span>
          </span>
