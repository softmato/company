...
import { MeetingSlotsCard, type MeetingSlot } from './chat-meeting-slots-card';
...
/**
 * Message ids, unique within a session and across reloads.
 *
 * `Date.now()` is impure and the React compiler refuses it anywhere it may
 * treat as render scope, which included the handlers that used to build these
 * ids inline. Reading the clock once at module load and counting from there is
 * pure at every call site, and still cannot collide with ids restored from
 * localStorage after a reload — a plain counter starting at zero would.
 */
...
function nextMessageId(prefix: string): string {
  messageSeq += 1;
  return `${prefix}-${SESSION_STAMP}-${messageSeq}`;
}
...
function ToolResultDataCard({
  ...
  if (
    ...
    return (
      <MeetingSlotsCard
        slots={toolData.availableSlots as MeetingSlot[]}
...
export function ChatWidget() {
   ...
   *
   * The setState below is synchronous and the rule is right that it costs one
   * extra render — but the alternatives are worse. A lazy `useState`
   * initialiser would read localStorage during render, and since the server
   * renders an empty conversation that is a hydration mismatch, which is a
   * correctness bug rather than a performance one. `useSyncExternalStore` with
   * a server snapshot would satisfy both, and is the right shape if this ever
   * needs to track storage changes; for a single read on mount it is more
   * machinery than the one render it saves.
   *
   * Suppressed deliberately, not overlooked.
   */
  useEffect(() => {
    try {
