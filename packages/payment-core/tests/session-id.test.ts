...
describe('session ids', () => {
  it('matches the database check constraint', () => {
    ...
    for (let i = 0; i < 200; i++) {
      expect(generateSessionId(i % 2 === 0 ? 'live' : 'test')).toMatch(
        constraint,
      );
    }
  });

  it('carries the mode of the credential, not of the server', () => {
    expect(generateSessionId('live').startsWith('cs_live_')).toBe(true);
    expect(generateSessionId('test').startsWith('cs_test_')).toBe(true);
  });

  it('carries at least 32 bytes of entropy', () => {
    // base64url of 32 bytes is 43 characters with no padding.
    const suffix = generateSessionId('live').slice('cs_live_'.length);
    expect(suffix.length).toBe(43);
  });

  it('does not repeat', () => {
    const seen = new Set<string>();
    for (let i = 0; i < 1000; i++) seen.add(generateSessionId('live'));
  ...
  it('rejects junk before it reaches a query', () => {
    ...
    expect(isSessionIdShape(generateSessionId('test'))).toBe(true);
  });
});
